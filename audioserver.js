...
app.post('/upload-audio', upload.single('audioFile'), async (req, res) => {
  ...
  try {
    ...
    await Promise.all(audioSegments.map(async (segment, index) => {
      try {
        // Upload the segment to GCS asynchronously using the improved function
        const segmentFileName = `<span class="math-inline">\{String\(file\.originalname\)\}\-part\-</span>{index}`;
        await uploadToGCS(bucketName, segmentFileName, segment);
        ...
        // Configure the audio source (use GCS URI)
        const audio = {
          uri: `gs://<span class="math-inline">\{bucketName\}/</span>{segmentFileName}`,
        };
